...
func (m *ClientInfo) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.SubscribedAt != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.SubscribedAt))
		i--
		dAtA[i] = 0x28
	}
	if len(m.ChanInfo) > 0 {
...
func (m *ClientInfo) SizeVT() (n int) {
	...
	if m.SubscribedAt != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.SubscribedAt))
	}
	n += len(m.unknownFields)
	return n
}
...
func (m *ClientInfo) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 5:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field SubscribedAt", wireType)
			}
			m.SubscribedAt = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.SubscribedAt |= int64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
